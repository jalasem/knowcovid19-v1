export default {
  majorStats: {
    total: {
      heading: 'Total Cases',
      quantity: 170122,
      info: '1st case: 27/02/2020'
    },
    hospitalized: {
      heading: 'Currently Hospitalized',
      quantity: 3240
    },
    deaths: {
      heading: 'Deaths',
      quantity: 2130
    },
    discharged: {
      heading: 'Discharged',
      quantity: 164752
    }
  },
  statesData: [
    { state: 'Lagos', total: 61569, recovered: 58515, death: 456 },
    { state: 'FCT', total: 19931, recovered: 19719, death: 168 },
    { state: 'Kano', total: 4006, recovered: 3896, death: 110 },
    { state: 'Ogun', total: 4726, recovered: 4646, death: 53 },
    { state: 'Osun', total: 2578, recovered: 2520, death: 52 },
    { state: 'Gombe', total: 2118, recovered: 2069, death: 44 },
    { state: 'Borno', total: 1344, recovered: 1305, death: 38 },
    { state: 'Katsina', total: 2114, recovered: 2055, death: 34 },
    { state: 'Edo', total: 4914, recovered: 4725, death: 185 },
    { state: 'Oyo', total: 6919, recovered: 6749, death: 128 },
    { state: 'Bauchi', total: 1549, recovered: 1532, death: 17 },
    { state: 'Kwara', total: 3162, recovered: 3068, death: 55 },
    { state: 'Akwa Ibom', total: 512, recovered: 391, death: 9 },
    { state: 'Kaduna', total: 9127, recovered: 9068, death: 65 },
    { state: 'Ekiti', total: 903, recovered: 867, death: 11 },
    { state: 'Taraba', total: 1001, recovered: 977, death: 24 },
    { state: 'Delta', total: 2657, recovered: 2556, death: 72 },
    { state: 'Ondo', total: 3500, recovered: 3413, death: 65 },
    { state: 'Rivers', total: 7418, recovered: 7274, death: 102 },
    { state: 'Enugu', total: 2484, recovered: 2435, death: 29 },
    { state: 'Niger', total: 935, recovered: 913, death: 17 },
    { state: 'Jigawa', total: 538, recovered: 512, death: 16 },
    { state: 'Zamfara', total: 244, recovered: 233, death: 8 },
    { state: 'Sokoto', total: 775, recovered: 747, death: 28 },
    { state: 'Abia', total: 1693, recovered: 1673, death: 20 },
    { state: 'Ebonyi', total: 2039, recovered: 2002, death: 32 },
    { state: 'Benue', total: 1366, recovered: 1327, death: 24 },
    { state: 'Anambra', total: 1909, recovered: 1826, death: 19 },
    { state: 'Plateau', total: 9068, recovered: 9008, death: 57 },
    { state: 'Adamawa', total: 1134, recovered: 1098, death: 32 },
    { state: 'Imo', total: 1661, recovered: 1624, death: 37 },
    { state: 'Yobe', total: 499, recovered: 490, death: 9 },
    { state: 'Nassarawa', total: 2385, recovered: 2345, death: 39 },
    { state: 'Kebbi', total: 450, recovered: 392, death: 15 },
    { state: 'Bayelsa', total: 907, recovered: 879, death: 26 },
    { state: 'Kogi', total: 5, recovered: 3, death: 2 },
    { state: 'cross River', total: 404, recovered: 384, death: 18 }
  ],
  helplines: [
    { state: 'Abia', helplines: ['07002242362', '0700ABIADOC'] },

    { state: 'Adamawa', helplines: ['08031230359', '07080601139'] },

    {
      state: 'Akwa Ibom',
      helplines: ['08028442194', '08037934966', '09023330092']
    },

    {
      state: 'Anambra',
      helplines: ['08145434416', '08030953771', '08117567363']
    },

    {
      state: 'Bauchi',
      helplines: [
        '08023909309',
        '08032717887',
        '08059600898',
        '0803368036',
        '08080330216',
        '08036911698'
      ]
    },

    {
      state: 'Bayelsa',
      helplines: ['08039216821', '07019304970', '08151693570']
    },

    { state: 'Benue', helplines: ['09018602439', '07025031214'] },

    { state: 'Borno', helplines: ['08088159881', '08009999999'] },

    {
      state: 'Cross River',
      helplines: ['09036281412', '08050907736-WhatsApp']
    },

    {
      state: 'Delta',
      helplines: ['08033521961', '08035078541', '08030758179', '09065031241']
    },

    {
      state: 'Ebonyi',
      helplines: ['08037433129', '08125915350', '09020332489']
    },

    { state: 'Edo', helplines: ['08084096723', '08064258163', '080358385529'] },

    {
      state: 'Ekiti',
      helplines: ['112', '09062970434', '09082970435', '09062970436']
    },

    { state: 'Enugu', helplines: ['117', '112', '08182555550'] },

    { state: 'FCT - Abuja', helplines: ['08099936312', '07080631500'] },

    { state: 'Gombe', helplines: ['07025116657', '07026256569'] },

    { state: 'Imo', helplines: ['08099555577', '07087110839'] },

    {
      state: 'Jigawa',
      helplines: ['08069323005', '08036440532', '08038806682', '07035188486']
    },

    {
      state: 'Kaduna',
      helplines: [
        '08025088304',
        '08032401473',
        '08035871662',
        '08037808191',
        '08036045755',
        '08097000010'
      ]
    },

    { state: 'Kano', helplines: ['09093995333', '09093995444'] },

    { state: 'Katsina', helplines: ['09035037114', '09047092428'] },

    {
      state: 'Kebbi',
      helplines: ['08036782507', '08036074588', '08032907601', '08067677723']
    },

    {
      state: 'Kogi',
      helplines: ['07088292249', '08150953486', '08095227003', '07043402122']
    },

    { state: 'Kwara', helplines: ['08064369063', '09029397826'] },

    { state: 'Lagos', helplines: ['08000CORONA', '0800027662'] },

    {
      state: 'Nasarawa',
      helplines: [
        '08036018579',
        '08035871718',
        '08033254549',
        '08121243191',
        '08032910826',
        '080097000010'
      ]
    },

    {
      state: 'Niger',
      helplines: ['08038246018', '09093093642', '08077213070']
    },

    { state: 'Ogun', helplines: ['08188978393', '08188978392'] },

    {
      state: 'Ondo',
      helplines: [
        '0800COVID19',
        '08002684319',
        '0700COVID19',
        '07002684319',
        '070012684319'
      ]
    },

    {
      state: 'Osun',
      helplines: ['293', '08035025692', '08033908772', '08056546250']
    },

    {
      state: 'Oyo',
      helplines: ['08095394000', '08095963000', '08078288999', '08078288800']
    },

    {
      state: 'Plateau',
      helplines: ['07032864444', '08035422711', '08065486416', '08035779917']
    },

    {
      state: 'Rivers',
      helplines: ['08056109538', '08031888093', '08033124314']
    },

    {
      state: 'Sokoto',
      helplines: ['07031936037', '08022069567', '08035074228', '08036294462']
    },

    {
      state: 'Taraba',
      helplines: ['08032501165', '08065508675', '08039359368', '08037450227']
    },

    { state: 'Yobe', helplines: ['08131834764', '07041116027'] },

    {
      state: 'Zamfara',
      helplines: ['08035626731', '08035161538', '08161330774', '08066408606']
    }
  ]
}
